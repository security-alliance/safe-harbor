...
contract DeployRegistryV2 is Script {
    ...
    address fallbackRegistry = 0x8f72fcf695523A6FC7DD97EafDd7A083c386b7b6;
    ...
    function run() public {
        ...
        SafeHarborRegistryV2 registry = new SafeHarborRegistryV2{salt: DETERMINISTIC_DEPLOY_SALT}(fallbackRegistry);
        vm.stopBroadcast();
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            keccak256(
                                abi.encodePacked(type(SafeHarborRegistryV2).creationCode, abi.encode(_fallbackRegistry))
                            )
                        )
                    )
                )
