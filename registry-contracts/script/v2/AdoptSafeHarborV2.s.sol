...
contract AdoptSafeHarborV2 is ScriptBase {
    ...
    address constant REGISTRY_V2_ADDRESS = address(0x00);

    function run() public {
        uint256 deployerPrivateKey = vm.envUint("DEPLOYER_PRIVATE_KEY");
        SafeHarborRegistryV2 registry = SafeHarborRegistryV2(REGISTRY_V2_ADDRESS);
        string memory json = vm.readFile("agreementDetails.json");

        adopt(deployerPrivateKey, registry, json);
    }

    function adopt(uint256 deployerPrivateKey, SafeHarborRegistryV2 registry, string memory json) public {
        ...
        // Deploy AgreementV2 contract
        AgreementV2 agreement = new AgreementV2(details, deployerAddress);
        registry.adoptSafeHarbor(address(agreement));
