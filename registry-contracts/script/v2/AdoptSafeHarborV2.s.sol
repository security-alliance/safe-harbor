...
import {AgreementDetailsV2, Chain as ChainV2, Account as AccountV2, Contact, BountyTerms, ChildContractScope, IdentityRequirements} from "../../src/v2/AgreementDetailsV2.sol";
...
contract AdoptSafeHarborV2 is ScriptBase {
    ...
    address constant FACTORY_ADDRESS =
        0x98D1594Ba4f2115f75392ac92A7e3C8A81C67Fed;
    ...
    function adopt(
        ...
        if (shouldAdoptToRegistry) {
            ...
            console.log(
                "Agreement created but not adopted to registry (ADOPT_TO_REGISTRY=false)"
            );
        }
    ...
    function parseAgreementDetails(
        string memory json
    ) internal view returns (AgreementDetailsV2 memory) {
        return
            AgreementDetailsV2({
    ...
    function parseContacts(
        string memory json
    ) internal view returns (Contact[] memory) {
        ...
        for (uint256 i = 0; i < contactCount; i++) {
            contacts[i] = Contact({
                name: json.readString(
                    string.concat(".contact[", vm.toString(i), "].name")
                ),
                contact: json.readString(
                    string.concat(".contact[", vm.toString(i), "].contact")
                )
            });
        }
        return contacts;
    ...
    function parseBountyTerms(
        string memory json
    ) internal pure returns (BountyTerms memory) {
        return
            BountyTerms({
                bountyPercentage: json.readUint(
                ...
                identity: IdentityRequirements(
                    uint8(json.readUint(".bountyTerms.identity"))
                ),
                diligenceRequirements: json.readString(
                    ".bountyTerms.diligenceRequirements"
                ),
                aggregateBountyCapUSD: json.readUint(
                    ".bountyTerms.aggregateBountyCapUSD"
                )
            });
    }
    ...
    function parseChains(
        string memory json
    ) internal view returns (ChainV2[] memory) {
        ...
        for (uint256 i = 0; i < chainCount; i++) {
            ...
            chains[i] = ChainV2({
                caip2ChainId: json.readString(
                    string.concat(".chains[", chainIndex, "].id")
                ),
                assetRecoveryAddress: json.readString(
                    string.concat(
                        ".chains[",
                        chainIndex,
    ...
    function parseAccounts(
        ...
        uint256 accountCount = getArrayLength(
            json,
            string.concat(".chains[", chainIndex, "].accounts"),
            ".accountAddress"
        );
    ...
    function getArrayLength(
        ...
        uint256 count = 0;
        while (true) {
            string memory fullPath = string.concat(
                arrayPath,
                "[",
                vm.toString(count),
                "]",
